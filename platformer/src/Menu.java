...
public class Menu {
    ...
    private static void loadSettingsMenu() {
        ...
        settingsMenu.add(new MenuSlider(BUTTON_GAP,100,500,100,"fps", "fps", 300, 10));
    ...
    private static void loadStatsMenu() {
        ...
        for (String stat : Stats.getExpectedStats()) {
            stats.add(new MenuText(50, 200+i*BUTTON_GAP, stat + ": " + ((Stats.doubleStats.contains(stat)) ? Stats.getD(stat) : Stats.get(stat)), 30, stat));
            i++;
        }
    ...
    private static void loadLevelMenu() {
        ...
        if (UserFileHandler.getCumulativeBestTimes() > 0) {
            levelMenu.add(new MenuText(25,50, "cumulative best times:    " + String.format("%.2f",UserFileHandler.getCumulativeBestTimes()), 15, "full"));
        }
